...
U, sigma,Vt=svds(mat_user_mean, k=5)
...
def recommend_recipes(df_svd,user_id,recipe_df,ratings_df,rec_num=5):
    ...
    recmd=recmd.rename(columns={user_row_num: 'Predictions'}).sort_values('Predictions',ascending='False')

    return user_history, recmd

#수정하기
"""
already_rated,predictions=recommend_recipes(df_svd,5,recipes,ratings,10)
