...
param_grid = {'n_epochs': [20,40], 'n_factors': [10,20,40,80]}
...
def get_recipes(df_svd,recipe_df,ratings_df):
    top_n=defaultdict(list)
    #for i in range(0, len(user_list)):
    for i in range(0, len(user_list)):
        if user_list[i]==0 : break;
        sorted_user_pred = df_svd.loc[i].sort_values(ascending=False)
        user_data = ratings_df[ratings_df.user_id == user_list[i]]
        ...
        recmd = recmd.rename(columns={i: 'Predictions'}).sort_values('Predictions', ascending=False).iloc[:3, :]

        top_n[i].append(recmd['recipe_id'].tolist())
        print(i," : ",top_n[i])
        del recmd
    return top_n
...
already_rated,predictions= recommend_recipes(df_svd,user_name,recipes,ratings,20)
print(already_rated)
print(predictions)



#preds=get_recipes(df_svd,recipes,ratings)
#print(preds)
